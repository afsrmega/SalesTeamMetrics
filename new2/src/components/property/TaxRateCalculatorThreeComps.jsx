...
const TaxRateCalculatorThreeComps = forwardRef(({ disabled }, ref) => {
  const { toast } = useToast();
  const [gba, setGba] = useLocalStorageState("property-tools:tax-rate-3comps:gba", "");
const [currentValue2024, setCurrentValue2024] = useLocalStorageState("property-tools:tax-rate-3comps:currentValue2024", "");
const [comp1SqFt, setComp1SqFt] = useLocalStorageState("property-tools:tax-rate-3comps:comp1SqFt", "");
const [comp2SqFt, setComp2SqFt] = useLocalStorageState("property-tools:tax-rate-3comps:comp2SqFt", "");
const [comp3SqFt, setComp3SqFt] = useLocalStorageState("property-tools:tax-rate-3comps:comp3SqFt", "");
const [estimatedOverpayment, setEstimatedOverpayment] = useLocalStorageState("property-tools:tax-rate-3comps:estimatedOverpayment", "");
const [calculationResult, setCalculationResult] = useState(null);
  ...
  return (
    <>
      <Card className="shadow-xl border-t-4 border-cyan-600 bg-white">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-2xl text-gray-800">
            <PercentSquare className="h-7 w-7 text-cyan-700" />
            Calculadora de Tax Rate (3 Comparables)
          </CardTitle>
          <CardDescription className="text-gray-600">
            Estima el Tax Rate basado en el valor de la propiedad y la mediana de 3 comparables.
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="gba-3comp" className="text-gray-700">Área (GBA)</Label>
                  <Input id="gba-3comp" type="number" placeholder="Ej: 2000" value={gba} onChange={(e) => setGba(e.target.value)} className="border-gray-300 focus:border-cyan-500 focus:ring-cyan-500" disabled={disabled} />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="currentValue2024-3comp" className="text-gray-700">Valor Actual 2024 ($)</Label>
                  <Input id="currentValue2024-3comp" type="number" placeholder="Ej: 500000" value={currentValue2024} onChange={(e) => setCurrentValue2024(e.target.value)} className="border-gray-300 focus:border-cyan-500 focus:ring-cyan-500" disabled={disabled} />
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="comp1SqFt-3comp" className="text-xs text-gray-600">Comparable 1</Label>
                  <Input id="comp1SqFt-3comp" type="number" placeholder="Ej: 250" value={comp1SqFt} onChange={(e) => setComp1SqFt(e.target.value)} className="border-gray-300 focus:border-cyan-500 focus:ring-cyan-500" disabled={disabled} />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="comp2SqFt-3comp" className="text-xs text-gray-600">Comparable 2</Label>
                  <Input id="comp2SqFt-3comp" type="number" placeholder="Ej: 260" value={comp2SqFt} onChange={(e) => setComp2SqFt(e.target.value)} className="border-gray-300 focus:border-cyan-500 focus:ring-cyan-500" disabled={disabled} />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="comp3SqFt-3comp" className="text-xs text-gray-600">Comparable 3</Label>
                  <Input id="comp3SqFt-3comp" type="number" placeholder="Ej: 240" value={comp3SqFt} onChange={(e) => setComp3SqFt(e.target.value)} className="border-gray-300 focus:border-cyan-500 focus:ring-cyan-500" disabled={disabled} />
                </div>
              </div>

              <div className="space-y-2">
                <Label htmlFor="estimatedOverpayment-3comp" className="text-gray-700">Sobrepago Estimado ($)</Label>
                <Input id="estimatedOverpayment-3comp" type="number" placeholder="Ej: 5000" value={estimatedOverpayment} onChange={(e) => setEstimatedOverpayment(e.target.value)} className="border-gray-300 focus:border-cyan-500 focus:ring-cyan-500" disabled={disabled} />
              </div>
            ...
            {calculationResult && (
              <motion.div
                ...
                className="bg-cyan-50 p-6 rounded-lg shadow relative"
              >
                <Button variant="ghost" size="icon" onClick={() => setIsModalOpen(true)} className="absolute top-2 right-2 text-gray-500 hover:text-cyan-600">
                  <Expand className="h-5 w-5" />
                </Button>
                <h3 className="text-xl font-semibold text-gray-800 mb-4 flex items-center">
                  <TrendingUp className="h-6 w-6 mr-2 text-cyan-700" />
        ...
        <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
          <DialogContent className="sm:max-w-[600px] bg-white rounded-lg shadow-xl p-0">
            <DialogHeader className="bg-gray-50 p-4 border-b">
              <DialogTitle className="text-xl font-semibold text-gray-800">Resultado Detallado: Tax Rate (3 Comps)</DialogTitle>
...
export default TaxRateCalculatorThreeComps;