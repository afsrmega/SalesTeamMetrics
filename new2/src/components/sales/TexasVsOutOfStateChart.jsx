...
const TexasVsOutOfStateChart = ({
  ...
  useEffect(() => {
    const fetchData = async () => {
      console.log('📊 [TexasVsOutOfState] Fetching Texas vs Out-of-State data...');
      console.log('  periodMode:', periodMode);
      ...
      try {
        ...
        if (periodMode === 'custom' && customStartDate && customEndDate) {
          ...
          label = "Custom Period";
        } else if (periodMode === 'quarter' && selectedQuarter) {
          let year, quarter;
          ...
          startDate = qDates.start;
          endDate = qDates.end;
        } else {
          // Default to month
          const baseDate = selectedMonth ? new Date(selectedMonth) : new Date();
        ...
        console.log(`📅 [TexasVsOutOfState] Date range (${label}): ${startDate.toISOString()} to ${endDate.toISOString()}`);

        let query = supabase
          .from('sales_records')
          .select('state, value, property_type')
          .eq('is_valid', true)
          .eq('is_deleted', false)
        ...
        records.forEach(record => {
          // JS fallback for includeResidential since is_residential column doesn't exist in DB schema
        ...
        console.log(`✅ [TexasVsOutOfState] Fetched ${records.length} total records`);
        console.log(`💰 [TexasVsOutOfState] Texas: ${formatCurrency(txTotal)} (${txCount}), Out: ${formatCurrency(outTotal)} (${outCount})`);

        setData({ txTotal, txCount, outTotal, outCount });
      } catch (err) {
        console.error('❌ [TexasVsOutOfState] Error fetching data:', err);
    ...
    fetchData();
  }, [selectedMonth, selectedQuarter, periodMode, includeResidential, customStartDate, customEndDate]);
...
export default TexasVsOutOfStateChart;